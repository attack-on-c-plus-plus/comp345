...
/*
 * Map is implemented as a connected graph, where each node represents a territory. Edges between nodes represent
 * adjacency between territories.
*/
class Map {
public:
    Map();
    ...
    Map &add(const Territory &territory);
    ...
    [[nodiscard]] const Continent &continent(const std::string &name) const;
    Map &add(const Continent &c);
    [[nodiscard]] size_t territoryCount() const;
    [[nodiscard]] size_t continentCount() const;
    [[nodiscard]] const Continent &continent(size_t id) const;
...
private:
    ...
    std::vector<const Continent*> *continents_;
    bool isConnectedGraph();
    [[nodiscard]] bool hasContinentSubGraphs() const;
    [[nodiscard]] bool hasOneContinentPerTerritory() const;
    void depthFirstSearchTerritory(std::vector<bool> &visited, size_t &count, unsigned vertex);
    friend std::istream &operator>>(std::istream &is, const Map &map);
...
private:
    ...
    const Player *owner_;
    unsigned *armyCount_;
    friend std::istream &operator>>(std::istream &is, const Territory &territory);
    friend std::ostream &operator<<(std::ostream &os, const Territory &territory);
    friend Map &Map::add(const Territory &territory);
};

/*
 * Continents are connected subgraphs, where each territory belongs to one and only one continent. Each territory is
 * owned by a player and contain a number of armies. The Map class can be used to represent any map configuration.
*/
class Continent {
public:
    Continent();
...
private:
    ...
    friend std::istream &operator>>(std::istream &is, const Continent &continent);
...
private:
    ...
    static mapReadState readMapSection(std::ifstream &, std::string &, const Map &);
